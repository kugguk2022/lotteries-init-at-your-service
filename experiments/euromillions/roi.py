...
def build_dataset(draw_df: pd.DataFrame, k: int, bias_cfg: BiasConfig, min_warmup: int):
    ...
    T = len(main)
    if T <= min_warmup + 1:
        raise ValueError("Not enough rows for warmup.")
...
def validate_prize_table(prize_table: dict):
    ...
    if missing:
        print(
            f"[warn] prize_table missing {len(missing)} tiers (set to 0): {missing[:6]}{'...' if len(missing) > 6 else ''}"
        )
    return prize_table
...
def inject_dynamic_jackpot(prizes_multi: list, jackpots: list, km=5, ks=2):
    ...
    print(
        f"[info] injected dynamic jackpot for {Lp} steps using {(Lj - off)} jackpot rows starting at offset {off}"
    )
    return prizes_multi
...
def eval_policy(
    ...
    return (
        np.array(hits_main, dtype=int),
        (cost, ret, roi),
        picks_idx_main,
        (picks_idx_star if has_stars else None),
    )
    ...
    final = Pipeline(
        [
            ...
            (
                "lr",
                LogisticRegression(
                    penalty="l2", C=C, class_weight="balanced", solver="lbfgs", max_iter=2000
                ),
            ),
        ]
    )
...
def run_all(
    ...
    p_star = (
        pd.read_csv(p_star_path, header=None).to_numpy(np.float64) if p_star_path.exists() else None
    )
    truth_star = (
        pd.read_csv(truth_star_path, header=None).to_numpy(np.int32)
        if truth_star_path.exists()
        else None
    )
    shapes = (
        [p_agent.shape[0], truth.shape[0], p_pair.shape[0]]
        + ([p_star.shape[0]] if p_star is not None else [])
        + ([truth_star.shape[0]] if truth_star is not None else [])
    )
    T = min(shapes)
    ...
    if hasattr(cfg, "jackpot_csv") and cfg.jackpot_csv and Path(cfg.jackpot_csv).exists():
        try:
            ...
            if prizes_multi is not None and len(prizes_multi) > 0:
                ...
                print(
                    "[warn] jackpot-csv provided but no prizes_multi in use; consider --prizes-multi file."
                )
    ...
    hits, (cost, ret, roi), picks_idx, picks_star = eval_policy(
        ...
        ev_mult=cfg.ev_mult,
        p_star=pS_hold,
        truth_star=truS_hold,
        dep_corr=cfg.dep_corr,
        weekly_steps=getattr(cfg, "weekly_steps", 2),
        risk_cap=getattr(cfg, "risk_cap", None),
        ledger_path=(outM / "roi_ledger.csv"),
    )
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="Max spend per week (EUR). If set, caps stakes per week window.",
    )
    ap.add_argument(
        "--weekly-steps", type=int, default=2, help="Number of draws per week window (default=2)."
    )
    ap.add_argument(
        "--jackpot-csv",
        type=str,
        default=None,
        help="CSV with per-draw jackpots to override prize (5,2) dynamically.",
    )
    # Stats
    ap.add_argument(
        "--permute-null",
