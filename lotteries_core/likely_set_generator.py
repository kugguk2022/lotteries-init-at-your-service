...
------------------------------------------------------------------------------------------------
A fair draw is independent of the past, so a co-occurrence score does NOT make any ticket more likely
to be drawn. G is a *descriptive* signature, and the level set is a *combinatorial equivalence class*,
not a set of "more probable winners." This tool exists to generate and study such sets forward-only --
to SEE whether the signal does anything against expectation -- not to promise wins. It moves no money.
"""
...
def build_portfolio(
    ...
    *,
    ...
    In ``predicted`` mode the matrices and target use only history BEFORE the last row (forward-only),
    so this is safe to call the moment before a draw to produce that draw's portfolio.
    """
    ...
    if target_mode == "observed":
        ...
        train = df.iloc[:-1] if len(df) > 1 else df
    ...
    return {"tickets": tickets, "target": target, "main_cols": main_cols, "star_cols": star_cols}
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.target_mode == "observed":
        ...
        # Forward-only: W from history BEFORE the last draw; target = trailing mean of past scores.
        train = df.iloc[:-1]
        W = build_comatrices(train, cfg, main_cols, star_cols)
